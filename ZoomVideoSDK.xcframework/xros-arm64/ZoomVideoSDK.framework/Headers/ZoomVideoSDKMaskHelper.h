...
/**
 * @brief Generate a mask UIImage based on the mask information that is located within the canvas width or the canvas height. The recommended aspect ratio is 1280 * 720 or 720 * 12880.
 ...
 * @param maskImage UIIage generate by interface "generateMask:width:height:". Or you can generate the bitmap by yourself. This mask image should meet the following conditions: The area through which you want to see the video should be set to opaque and the other areas should be set to transparent.
 * @param backgroundImage The background iamge.
 * @param isMirrored Whether the video that is based by the mask generate is mirrored. For \link ZoomVideoSDKVideoCanvas \endlink, the value \link ZoomVideoSDKVideoHelper::isMyVideoMirrored \endlink can be passed in. For \link ZoomVideoSDKRawDataPipe \endlink, the correct value should be passed.
 * @return errorCode
 */
